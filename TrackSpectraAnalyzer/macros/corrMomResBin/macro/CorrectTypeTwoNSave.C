...
void CorrectTypeTwoNSave(const char *cDir="../root_files/",
			 const char *cFile="CORR_MB-C10-PR9-Merged_proc0710_pileup",
			 const char *cFileTRK1="TrkHistMC_july09v2_qcdPt170dJuly09",
			 ...
			 )
{
   ...
   sprintf(inFile,"./rootOutput/%s.root",cFile);
   ...
   if(!rebOnly){
	 ...
	 hRes3D->GetXaxis()->GetFirst() : hRes3D->GetXaxis()->FindBin(-1.0*feta+small);
      int eta_max_bin = (feta>=hRes3D->GetXaxis()->GetXmax()) ?
	 hRes3D->GetXaxis()->GetLast() : hRes3D->GetXaxis()->FindBin(feta-small);
   ...
   dum4 = GetDummyHist(0.5,200,0.7,2.8,"p_{T}^{rec}","Correction factor",false);
   dum4->Draw("");

   TLine *line = new TLine(0.5,1,200,1);
   line->SetLineStyle(2);
   line->SetLineWidth(2);
   line->Draw();
   ...
   leg4 = new TLegend(0.41,0.34,0.85,0.72);    
   leg4->SetMargin(0.3), leg4->SetBorderSize(1);
   leg4->SetFillColor(kWhite), leg4->SetFillStyle(0);
   leg4->SetTextSize(0.039), leg4->SetMargin(0.2);

   leg4->AddEntry(hCorrLev1,"binning corr.","p");
   leg4->AddEntry(hCorrLev1R,"binning corr. (rebinned)","p");
   leg4->AddEntry(hCorrLev2,"binning/res. corr.","p");
   leg4->AddEntry(hCorrLev2R,"binning/res. corr. (rebinned)","p");
   leg4->Draw();
