...
HackedAnalyticalTrackSelector::HackedAnalyticalTrackSelector( const edm::ParameterSet & cfg ) :
    ...
    chi2n_par_pt_(cfg.getParameter< std::vector<double> >("chi2n_par_pt")),
    chi2n_par_pt2_(cfg.getParameter< std::vector<double> >("chi2n_par_pt2")),
...
HackedAnalyticalTrackSelector::~HackedAnalyticalTrackSelector() {
}

void HackedAnalyticalTrackSelector::produce( edm::Event& evt, const edm::EventSetup& es ) 
{
    using namespace std; 
    using namespace edm;
...
bool HackedAnalyticalTrackSelector::select(const reco::BeamSpot &vertexBeamSpot, const reco::Track &tk, const std::vector<Point> &points, std::vector<double> &vterr, std::vector<double> &vzerr) {
  // Decide if the given track passes selection cuts.
   ...
   // pt dependent
    ...
    double chi2n_par_pt_cut2
       = chi2n_par_pt2_[0] - chi2n_par_pt2_[1]/(1.0 + TMath::Exp(-chi2n_par_pt2_[2]*(pt-chi2n_par_pt2_[3]))) + 1.73/pow((pt+3.5),1.4);
...
void HackedAnalyticalTrackSelector::selectVertices(const reco::VertexCollection &vtxs, std::vector<Point> &points, std::vector<double> &vterr, std::vector<double> &vzerr) {
