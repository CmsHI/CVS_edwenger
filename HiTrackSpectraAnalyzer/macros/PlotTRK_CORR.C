...
void PlotTRK_CORR(){
   ...
   if(lowpT){
      pt_min = 0.9, pt_max = 15;
      ymin = 6E-4, ymax = 5E+3;
      ymin_r = 0.37, ymax_r = 1.23;
   }else{
      pt_min = 0.9, pt_max = 90;
      ymin = 6E-8, ymax = 5E+3;
      ymin_r = 0.37, ymax_r = 2.23;
   }
   //------------------ file loading

   // GEN truth
   specTruthF = (TFile*) loadFile(specTruthF,
				  //"./rootOutput/CORR_HiTrkHistMC_july09_hydjetMultJuly09_eta_0.0to2.4_jet_0.0to2000.0_hitrackAna_GEN1_varBin1.root");
				  "./rootOutput/CORR_HiTrkHistMC_july08_hydjet_small_v2_eta_0.0to2.4_jet_0.0to2000.0_hitrackAna_GEN1_varBin1.root");
   ...
   specMC_reco = (TFile*) loadFile(specMC_reco,
				   //"./rootOutput/CORR_HiTrkHistMC_july09_hydjetMultJuly09_eta_0.0to2.4_jet_0.0to2000.0_hitrackAna_GEN0_varBin1.root");
				   "./rootOutput/CORR_HiTrkHistMC_july08_hydjet_small_v2_eta_0.0to2.4_jet_0.0to2000.0_hitrackAna_GEN0_varBin1.root");
				   //"./rootOutput/CORR_HiTrkHistMC_july09_hydjetMultJuly09_eta_0.0to2.4_jet_0.0to2000.0_hitrackAna_GEN0_varBin1.root");
   ...
   // Rebin and etc      
   int rebinF = 3;
   hdndpt_truth->Rebin(rebinF);
   hdndptMC_recoRaw->Rebin(rebinF);
   hdndptMC_recoLev1->Rebin(rebinF);
   hdndptMC_recoLev2->Rebin(rebinF);
   hdndptMC_recoFull->Rebin(rebinF);


   // Normalization
   float N_evt = 80610;
   hdndpt_truth->Scale(1./(N_evt*rebinF));
   hdndptMC_recoRaw->Scale(1./(N_evt*rebinF));
   hdndptMC_recoLev1->Scale(1./(N_evt*rebinF));
   hdndptMC_recoLev2->Scale(1./(N_evt*rebinF));
   hdndptMC_recoFull->Scale(1./(N_evt*rebinF));

   //hdndpt_truth = (TH1D*) rerangedHist(hdndpt_truth,4);
   int binMin = 7;
   hdndptMC_recoRaw = (TH1D*) rerangedHist(hdndptMC_recoRaw,binMin);
   hdndptMC_recoLev1 = (TH1D*) rerangedHist(hdndptMC_recoLev1,binMin);
   hdndptMC_recoLev2 = (TH1D*) rerangedHist(hdndptMC_recoLev2,binMin);
   hdndptMC_recoFull = (TH1D*) rerangedHist(hdndptMC_recoFull,binMin);
   ...
   hdndpt_truth_dum = (TH1D*) rerangedHist(hdndpt_truth_dum,binMin); 
   hdndptMC_recoRaw_dum = (TH1D*) rerangedHist(hdndptMC_recoRaw_dum,binMin);
   hdndptMC_recoLev1_dum = (TH1D*) rerangedHist(hdndptMC_recoLev1_dum,binMin);
   hdndptMC_recoLev2_dum = (TH1D*) rerangedHist(hdndptMC_recoLev2_dum,binMin);
   hdndptMC_recoFull_dum = (TH1D*) rerangedHist(hdndptMC_recoFull_dum,binMin);
   ...
   if(lowpT){  
      printCanvases(call,"spectraPbPb_TRK_CORR_lowpT",0,1);
   }else{
      printCanvases(call,"spectraPbPb_TRK_CORR_highpT",0,1);
   }
...
void drawLegend(){
   ...
   legd->SetHeader("PbPb #rightarrow h+X, 2.76 TeV MC, |#eta|<2.4");
   legd->AddEntry(hdndpt_truth,"HYDJET MC","l"); 
