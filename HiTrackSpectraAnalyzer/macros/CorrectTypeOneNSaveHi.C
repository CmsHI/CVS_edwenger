...
//    3. full -> trk+sec+mult
//
// *Modified for HI spectra analysis with a name change to CorrectTypeOneNSaveHi.C 
//  July 10? 2010 - Andre
//-------------------------------------------------------------------------------------
...
void CorrectTypeOneNSaveHi(const char *cDir="../root_files/",
                         ...
                         )
{
   ...
   for(int j=0;j<(nbinX*nbinY*nbinZ);j++){
	 ...
	 if(eff== -999) eff = 1; // meaning no correction
	 if(fak== -999) fak = 0; // meaning no correction 
	 ...
	 // secondary !
	 // if sec = 1 --> it kills! any protection?
	 if(sec==-999) sec = 0; //meaning no correction 
	 ...
	 if(mlt==-999) mlt = 0; // meaning no correction
...
void drawDebugPlots(){
   ...
   dum = GetDummyHist(200,1E-2,1E+9,xTitle,yTitle);
   dum->Draw();
...
double GetEffFactor(TH3F* num, TH3F* den, double pt, double eta, double jet){
   // two cases where eff = 0
   // 1. no entries (num =0 && den = 0) => assign eff=-999 (no correction)
   // 2. no macth (num = 0 but den =! 0) => assign eff=-999 (no correction)
   // * two cases are treated identically
   ...
   if(n_den == 0  || n_num == 0 ) return -999;
   else return n_num/n_den;
}

double GetFakFactor(TH3F* num, TH3F* den, double pt, double eta, double jet){
   // two cases where fake = 0
   // 1. no entries (num=0 && den =0) => assign fake = -999
   // 2. no match (num=0 bu den=! 0) => assign num/den
   // * in the end, two cases are treated identically!
   // 1. finite entries => fake = [0,1] (inclusive)
   // 2. no entries => no correction! (fake = 0)
   ...
   //if(n_den == 0 && n_num ==0) return 999;
   if(n_den == 0) return -999; // meaning nothing is reconstructed or no entries
   else return n_num/n_den;
