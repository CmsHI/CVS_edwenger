...
create_SubtractorPed(const edm::ParameterSet& conf) {
  int ac = conf.getParameter<int>("addConstant");
  int mts = conf.getParameter<int>("minTruncStrips");
  int mt = conf.getParameter<int>("maxTries");
  return std::auto_ptr<SiStripPedestalsSubtractor>( new SiStripPedestalsSubtractor(ac,mts,mt) );
}
